...
def test_sample_create(astore_client):
...
def test_duplicate_sample(astore_client):
    ...
    pytest.raises(HTTPError, astore_client.create_sample, name='test_duplicate',
                  uid=_com_uid)
...
def test_update_sample(astore_client):
    ...
    astore_client.update_sample(query={'uid': test_sample['uid']},
...
def test_update_sample_illegal(astore_client):
    ...
    pytest.raises(HTTPError,
                  astore_client.update_sample, query={'name': test_sample['name']},
                                      update={'uid': 'illegal'})
...
def test_update_container(astore_client):
    ...
    astore_client.update_container(query={'uid': orig_cont['uid']},
                          update={'state': 'inactive','time': ttime.time()})
...
def test_update_container_illegal(astore_client):
    ...
    pytest.raises(HTTPError, astore_client.update_container, query={'uid': orig_cont['uid']},
                  update={'uid': str(uuid.uuid4())})


def test_request_create(astore_client):
    astore_client.create_request(sample='roman_sample', time=ttime.time(),
...
def test_duplicate_request(astore_client):
    ...
    astore_client.create_request(sample='hidefix', time=ttime.time(),
               uid=m_uid, state='active', seq_num=0, foo='bar',
               hero='asterix', antihero='romans')
    pytest.raises(HTTPError, astore_client.create_request, sample='hidefix', time=ttime.time(),
               uid=m_uid, state='active', seq_num=0, foo='bar',
               hero='asterix', antihero='romans')
...
def test_update_request(astore_client):
    ...
    astore_client.update_request(query={'uid': m_uid},
